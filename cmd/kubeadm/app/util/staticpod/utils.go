...
func ComponentProbe(cfg *kubeadmapi.MasterConfiguration, component string, port int, path string, scheme v1.URIScheme) *v1.Probe {
	return &v1.Probe{
		Handler: v1.Handler{
...
func GetProbeAddress(cfg *kubeadmapi.MasterConfiguration, component string) string {
	switch {
	case component == kubeadmconstants.KubeAPIServer:
		if cfg.API.AdvertiseAddress != "" {
			return cfg.API.AdvertiseAddress
		}
	case component == kubeadmconstants.KubeScheduler:
	...
	case component == kubeadmconstants.KubeControllerManager:
	...
	case component == kubeadmconstants.Etcd:
